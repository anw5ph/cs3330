...
stall_F = loadUse || (f_Stat != STAT_AOK && f_Stat != STAT_HLT) || ((D_icode == RET || E_icode == RET || M_icode == RET ) && !(M_conditionsMet == 0 && M_ifun != 0 && M_icode == JXX ));
...
pc = [
    W_icode == RET : W_valM;
    M_conditionsMet == 0 && M_ifun != 0 && M_icode == JXX :  F_pc;
    1 : F_predPC;
];


f_pc = [
    ((f_ifun != 0) && f_icode == JXX) : f_valP;
    1 : F_pc;
];
...
stall_D = loadUse || (E_icode == POPQ && (E_rA == d_srcA || E_rB == d_srcB));
bubble_D = ((D_icode == RET || E_icode == RET || M_icode == RET) && !(M_conditionsMet == 0 && M_ifun != 0 && M_icode == JXX) && !loadUse && !(E_icode == POPQ && (E_rA == d_srcA || E_rB == d_srcB)));
...
d_valA = [
    ...
    (reg_srcA == m_dstM) && (M_icode == MRMOVQ) : m_valM;
    (reg_srcA == W_dstM) && (W_icode == MRMOVQ) : W_valM;
...
d_valB = [
    ...
    (reg_srcB == m_dstM) && (M_icode == MRMOVQ) : m_valM; # forward post-memory (M -> E)
    (reg_srcB == W_dstM) && (W_icode == MRMOVQ) : W_valM; # forward pre-writeback ("register file forwarding")
...
bubble_E = loadUse || (E_icode == RET || M_icode == RET) || (M_ifun != 0 && M_icode == JXX && M_conditionsMet == 0);
...
e_conditionsMet = [
    (E_icode == JXX) && (E_ifun != 0): !conditionsNotMet;
    1 : M_conditionsMet;
];

e_dstM = [
    (E_icode == CMOVXX) && (E_ifun != 0) && conditionsNotMet: REG_NONE;
    1 : E_dstM;
];
...
bubble_M = (M_icode == RET) || (M_icode == JXX && M_ifun != 0 && M_conditionsMet == 0);
